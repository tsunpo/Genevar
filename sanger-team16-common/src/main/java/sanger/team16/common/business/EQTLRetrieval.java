 ...
 * @date   08/06/12
 */
@WebService(endpointInterface = "sanger.team16.service.EQTLRetrievalService", serviceName = "EQTLRetrieval")
public class EQTLRetrieval extends AbstractRetrieval implements EQTLRetrievalService
{  
    public EQTLRetrieval() {
        super();
    }
    
    public EQTLRetrieval(String address) {
        super(address);
    }
     ...
     */
    @WebMethod
    public QTLList getEQTLsWhereGene(int studyId, int genotypeFeatureId, int expressionFeatureId, int transcriptId, String chromosome, int transcriptionStartSite, int statisticId, double threshold, int distance) {        
        QTLList qtls = new QTLList();
        List<Variation> snps = new VariationDAO(address).list(studyId, chromosome, transcriptionStartSite, distance);
     ...
     */
    @WebMethod
    public List<QTL> getEQTLsWhereSNP(int genotypeFeatureId, Variation snp, int expressionFeatureId, List<TranscriptMapping> transcriptMappings, int statisticId, double threshold) {
        ...
        if (transcriptMappings != null) {
            ...
            if (statisticId > Statistic.EXTERNAL_ALGORITHM) {
                List<AnalysisFeature> analysisFeatures = new AnalysisFeatureDAO(address).listEQTL(statisticId, genotypeFeatureId, expressionFeatureId);
                //AnalysisDAO analysisDAO = new AnalysisDAO(address);   //REMOVE 23/09/13 //ADD 23/11/10
             ...
             */
            	...
            	//TupleDAO tupleDAO = new TupleDAO(address);   //REMOVE 23/09/13 //ADD 15/06/12

                for (int i=0 ; i<transcriptMappings.size() ; i++) {
                    TranscriptMapping transcriptMapping = transcriptMappings.get(i);
                    Tuple tuple = new TupleDAO(address).getTupleGxE(genotypeFeatureId, snp.getId(), expressionFeatureId, transcriptMapping.getTranscriptId());
 
                    QTLStatistics stats = new QTLStatistics();
                    if (stats.calculate(tuple, snp.getAlleles(), statisticId, threshold, 0))
                        qtls.add(new QTL(transcriptMapping, stats.nominalP));
                }
            }
        }
     ...
     */
    @WebMethod
    public Tuple getEQTLTuple(int genotypeFeatureId, Variation snp, int expressionFeatureId, int transcriptId, int statisticId, int permutation) {
        Tuple tuple = new TupleDAO(address).getTupleGxE(genotypeFeatureId, snp.getId(), expressionFeatureId, transcriptId);
         ...
         */
            ...
            QTLStatistics stats = new QTLStatistics();
            if (stats.calculate(tuple, snp.getAlleles(), statisticId, 1, permutation)) {
                tuple.a1 = stats.a1;
                tuple.a2 = stats.a2;
                tuple.setSubtitle(stats.getSubtitle(statisticId, permutation));
            }
        }
