...
{
    public ExpressionJDBC(String address) {
        super(address);
    }
    ...
    public void loadDataLocalInfile(int featureId, String fileName) {
        ...
        String sql = "LOAD DATA LOCAL INFILE '" + fileName + "'" +
            " INTO TABLE expression (transcript_id, individual_id, value)";
        if (this.address.equals("tpy_team16_genevar_2_0_0.cfg.xml"))
            sql += " SET population_expression_id = ";
        else
            sql += " SET expression_feature_id = ";
        sql += featureId;
