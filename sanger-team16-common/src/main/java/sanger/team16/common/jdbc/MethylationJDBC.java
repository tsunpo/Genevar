 ...
 */
public class MethylationJDBC extends AbstractJDBC
{
    public MethylationJDBC(String address) {
        super(address);
    }
    ...
    public void loadDataLocalInfile(int featureId, String fileName) {
        ...
        String sql = "LOAD DATA LOCAL INFILE '" + fileName + "'" +
            " INTO TABLE methylation (modification_id, individual_id, value)";
        if (this.address.equals("tpy_team16_genevar_2_0_0.cfg.xml"))
            sql += " SET population_methylation_id = ";
        else
            sql += " SET methylation_feature_id = ";
        sql += featureId;
