...
public class ExpressionLoadPane extends AbstractManagerPane implements ActionListener
{
    ...
    public ExpressionLoadPane(UI ui) {
        super(ui);
    ...
    public void refresh(int studyIndex) {
        ...
        this.setLoadToStudyPanel(studyIndex);
        this.setBlankPanel();
    ...
    private void setLoadToStudyPanel(int studyIndex) {
        BaseJPane panel = new BaseJPane("Load Expression Profiling Data into Local Database");
        ...
        p2.add(new JLabel("3. Expression profiling file: "));
        
        tfFile = new BaseJTextField(15, true, true);
        //tfFile1.setText("/nfs/team16/tpy/dev/mysql/ceu.norm.smv9_10.txt");   // FOR TEST!!
        p2.add(tfFile);
    ...
    public void loadPaneActionPerformed(ActionEvent ae) {
        ...
        if (ae.getSource() == cbStudy) {
            ...
            if (cbStudy.getSelectedIndex() != 0) {
                ...
                studyPlatforms.add(this.ui.getPlatform(study.getExpressionPlatformId()));
            ...
            new ExpressionLoadPaneProgressBar().execute();
     ...
     * @link   http://www.sanger.ac.uk/Teams/Team16/
     */
    private class ExpressionLoadPaneProgressBar extends SwingWorker<Object, Object>
    {   
        ...
        protected Void doInBackground() {
            ...
            try {
                List<Integer> expressionFeatureIds = getMethylationFeatureIds(populationId, platformId, tissueTypeId, methodId);
                
                Object[] options = {"  Yes  ", "   No   "};
                if (expressionFeatureIds.size() == 0) {   // New ExpressionFeature!!
                    int answer = JOptionPane.showOptionDialog(ui, "Load \'" + description + "\' into the database?", "Confirm", JOptionPane.YES_NO_OPTION, JOptionPane.QUESTION_MESSAGE, null, options, options[1]);
                    if (answer == JOptionPane.YES_OPTION)
                        this.load(null);
                
                } else {                                  // ExpressionFeature already exists!!
                    ...
                    if (answer == JOptionPane.YES_OPTION)
                        this.load(expressionFeatureIds);
                }
        ...
        private void load(List<Integer> expressionFeatureIds) throws IOException, NumberFormatException {
            ...
            // 1. Create a new ExpressionFeature
            int newExpressionFeatureId = new ExpressionFeatureDAO(ui.getAddress()).save(new ExpressionFeature(populationId, platformId, tissueTypeId, methodId, description, "tpy"), true);

            // 2. Load profile into Expression
            ExpressionLoader expressionLoader = new ExpressionLoader(ui.getAddress(), newExpressionFeatureId, populationId, platformId);
            expressionLoader.load(tfFile.getText(), probeIdColumn, individualColumn);
            
            transcript = expressionLoader.transcript;
            individual = expressionLoader.individualIds.size();
            
            // 3. Update the log part of ExpressionFeature
            new ExpressionFeatureDAO(ui.getAddress()).updateExpressionFeature(newExpressionFeatureId, tfFile.getText(), individual, transcript);
            if (expressionFeatureIds != null)
                new ExpressionFeatureDAO(ui.getAddress()).updateParentFeatureId(expressionFeatureIds);
        }
        ...
        protected void done() {
            ...
            String upload = individual + " individuals and " + transcript + " transcripts uploaded";
