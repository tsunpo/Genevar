...
import sanger.team16.gui.genevar.mqtl.GeneTableModel;
...
public class CisMQTLGenePane extends MQTLAnalysisPane implements ActionListener
{
    ...
    private void setQueryGenePanel(SelectedIndex selectedIndex, boolean searched) {
        BaseJPane panel = new BaseJPane("Query CpGs");
        ...
        p2.add(new JLabel("3. Enter probe ID/CpG island/gene symbol(s): "));
        ...
        p3.add(new JLabel("    or read from a text file with list of CpGs: "));
    ...
    private void cisGenePaneActionPerformed(ActionEvent ae) {
    	...
    	int platformId = ((Study) cbStudy.getSelectedItem()).getMethylationPlatformId();
        ...
        if (ae.getSource() == cbStudy) {
            ...
            this.geneTable = new GeneTableModel(this.getCpGs(platformId, referenceId, inputKeeper.inputs, matched)).getTable();
            ...
            try {   //ADD 19/11/10
                ...
                for (int i=0 ; i<row ; i++)
                    if ((Boolean) geneTable.getValueAt(i, 0)) {
                    	String probeId = (String) geneTable.getValueAt(i, 1);
                    	String cpgIslands = (String) geneTable.getValueAt(i, 2);
                    	String geneSymbol = (String) geneTable.getValueAt(i, 5);
                    	String title = "";
                        ...
                        if (!probeId.equals(""))
                        	title = probeId;
                           	...
                           	if (!geneSymbol.equals(""))
                        		title = geneSymbol;
                        	if (!cpgIslands.equals("") & geneSymbol.equals(""))
                        		title = cpgIslands; 	
                        }
   
                        List<ModificationMapping> modificationMappings = getModificationMappings(study.getMethylationPlatformId(), referenceId, title);
                        ui.addToCurrentNode(title, new CisMQTLGenePane2(ui, study, modificationMappings));
                    }
