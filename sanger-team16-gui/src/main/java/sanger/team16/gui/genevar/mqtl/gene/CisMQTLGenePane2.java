...
public class CisMQTLGenePane2 extends MQTLAnalysisPane implements ActionListener
{
    private Study study;
    private JComboBox cbDistance, cbPValue;
    private BaseJTable modificationTable, matchedFeatureTable;
    private Vector<Algorithm> algorithms;
    private boolean isDefault = true;
    
    public CisMQTLGenePane2(UI ui, Study study, List<ModificationMapping> modificationMappings) {
        super(ui);
        this.study = study;
        this.modificationTable = new ModificationTableModel(true, modificationMappings).getTable();
        this.matchedFeatureTable = new MatchedFeatureTableModel(this.getMatchedFeaturesGxM(study.getId())).getTable();
    ...
    public void refresh(SelectedIndex selectedIndex) {
        ...
        this.setBlankPanel();
        this.setParameterPanel(selectedIndex, "2. Distance to Probe Start: ");
        this.setBlankPanel();
    ...
    private void setTranscriptPanel() {
        BaseJPane panel = new BaseJPane("Matched Modification(s)");

        // ------------------ Panel p0 (start) ------------------ //
        BaseJPane p0 = new BaseJPane();

        p0.add(new JLabel("Platform: "));
        p0.add(new BaseJLabel(this.ui.getPlatform(this.study.getMethylationPlatformId()).getName(), new Font("Arial", Font.BOLD, 13)));
        ...
        p1.add(modificationTable.getTableHeader(), BorderLayout.PAGE_START);
        p1.add(modificationTable, BorderLayout.CENTER);
        ...
        if (ae.getSource() == bRemove)
            ...
            if (distance > 100000)
                JOptionPane.showMessageDialog(ui, "No greater than 100 kb",
                    "Distance to Probe Start", JOptionPane.WARNING_MESSAGE);
            else
            ...
            try {   //ADD 19/11/10
                for (int i=0 ; i<matchedFeatureTable.getRowCount() ; i++)
                    if (((Boolean) matchedFeatureTable.getValueAt(i, 0)).booleanValue()) {
                        ...
                        ui.addToCurrentNode(treeNoteName, new CisMQTLGenePane3(ui, treeNoteName, modificationTable, study, matchedFeature, statisticId, algorithms, distance, threshold));   
                    } 
