...
import sanger.team16.gui.genevar.mqtl.MatchedFeatureTableModel;
...
public class CisMQTLSNPPane extends MQTLAnalysisPane implements ActionListener
{
    private Study study;   //TODO only in cis-SNP
    ...
    public void setSubmitSNPPanel(SelectedIndex selectedIndex) {
        ...
        cbReference = new JComboBox(this.ui.getReferences(study.getGenotypeAssemblyId(), study.getMethylationPlatformId()));
    ...
    public void setParameterPanel(SelectedIndex selectedIndex, String distanceTo) {
        ...
        // ------------------ Panel p0 (start) ------------------ //
        BaseJPane p0 = new BaseJPane();
        algorithms = this.getQTLAlgorithms(study.getId(), "M");
        ...
        cbDistance = new JComboBox(distances);   //TODO
        cbDistance.setEditable(true);
        cbDistance.addActionListener(this);
        p1.add(cbDistance);
        p1.add(new BaseJLabel("(Editable, no greater than 100 kb)", Color.GREEN.darker()));
    ...
    private void cisSNPPaneActionPerformed(ActionEvent ae) {
        ...
        if (ae.getSource() == cbStudy) {
            ...
            if (distance > 100000)
                JOptionPane.showMessageDialog(ui, "No greater than 100 kb",
    ...
    {   
        ...
        protected Void doInBackground() {
        	int assemblyId = ((Reference) cbReference.getSelectedItem()).getAssemblyId();  //TODO
        	int referenceId = ((Reference) cbReference.getSelectedItem()).getId();   //TODO
            ...
            try {
                for (int i=0 ; i< inputSNPs.size(); i++) {
                    ...
                    if (snp != null) {
                        ...
                        List<ModificationMapping> modificationMappings = getModificationMappingsWhereSNP(study.getMethylationPlatformId(), referenceId, snp, distance);
           
                        RegionalLinePlot regionalLinePlot = createRegionalLinePlot(snp, modificationMappings, sigEQTLs, statisticId, threshold);                        
                        ...
                        ui.addToCurrentNode(treeNoteName, new CisMQTLSNPPane2(ui, snp, matchedFeatureTable, reginalLinePanel, modificationMappings, sigEQTLs, assemblyId, distance, statisticId, algorithms, treeNoteName)); 

                    } else
                        ui.addToCurrentNode(variationName, new CisMQTLSNPPane2(ui, variationName));
                }
        ...
        private RegionalLinePlot createRegionalLinePlot(Variation snp, List<ModificationMapping> modificationMappings, List<String> sigEQTLs, int statisticId, double threshold) {
            ...
            for (int i=0 ; i<matchedFeatureTable.getRowCount() ; i++)
                if (((Boolean) matchedFeatureTable.getValueAt(i, 0)).booleanValue()) {
                    ...
                    List<QTL> eqtls = getMQTLsWhereSNP(matchedFeature, snp, modificationMappings, statisticId, threshold);
                    
                    if (eqtls != null && eqtls.size() != 0) {   // BUG 20/10/10
                        sigEQTLs.addAll(this.getSigMQTLs(eqtls, threshold));
        ...
        private List<String> getSigMQTLs(List<QTL> mqtls, double threshold) {
            List<String> significants = new ArrayList<String>();
            
            if (mqtls != null && mqtls.size() != 0)   // BUG 20/10/10
                for (int i=0 ; i<mqtls.size() ; i++) {
                    QTL eqtl = mqtls.get(i);
                    
                    if (eqtl.nominalP <= threshold)
                        significants.add(eqtl.modificationMapping.getProbeId());
                }
